...
export default class FormElementsEdit extends React.Component {
  ...
  onEditorStateChange(index, property, editorContent) {
    ...
    // const html = draftToHtml(convertToRaw(editorContent.getCurrentContent())).replace(/<p>/g, '<span>').replace(/<\/p>/g, '</span>').replace(/&nbsp;/g, '');
    // console.log(html);
    // const html = draftToHtml(convertToRaw(editorContent.getCurrentContent())).replace(/<p>/g, '<span>').replace(/<\/p>/g, '</span>').replace(/&nbsp;/g, '')
    //   .replace(/(?:\r\n|\r|\n)/g, '');
      ...
      <div>
        ...
        { this.props.element.hasOwnProperty('label') &&
          <div className="form-group">
            ...
            { (this.state.element.element === 'TextInput' || this.state.element.element === 'NumberInput') &&
...
{ 
            <div className="custom-control custom-btn">
           <button class="btn btn-primary" type="button" onClick={this.props.manualEditModeOff}><i class='fas fa-save'></i>Save</button>
            </div>}
      </div>
    );
  }
