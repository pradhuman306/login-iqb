...
function QuestionnaireAccessManagement(props) {
  ...
  const checkUserinList = (user, filtertext) => {
    let checkReturn = false;
    user.forEach((element) => {
      if (element.firstname.toLowerCase().includes(filtertext.toLowerCase())) {
        checkReturn = true;
      }
    });
    return checkReturn;
    ...
    if (filterText) {
      let tmp = Permissions.filter((item) => {
        if (
          ...
          checkUserinList(item.users, filterText.toLowerCase())
        ) {
          return true;
        }
        return false;
        ...
        return (
          <div className="user-wrap">
            <h5 className="user-icon">
              {row.firstname[0] ? row.firstname[0].toUpperCase() : ""}
              {row.lastname[0] ? row.lastname[0].toUpperCase() : ""}
            </h5>
